...
module.exports = {
    ...
    async execute(message, args) {
        if (args.length < 1) {
            message.channel.send('Usage: !restart <server>');
            return;
        }
        ...
        try {
            ...
            message.channel.send(`Server ${serverName} is restarting...`);

            rcon.close();
        } catch (error) {
            console.error('Error restarting server:', error);
            message.channel.send('Failed to restart server. Is the server online?');
        }
    }
};
