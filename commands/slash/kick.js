...
module.exports = {
    ...
    async execute(interaction, serversRconConfig, serverName) {
        ...
        try {
            ...
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({ content: 'Failed to kick player. Is the server online?', ephemeral: true });
            } else {
                await interaction.reply({ content: 'Failed to kick player. Is the server online?', ephemeral: true });
            }
        }
    }
};
