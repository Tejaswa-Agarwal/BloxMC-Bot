...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('broadcast')
        .setDescription('Broadcast a message to all players on the Minecraft server')
    ...
    async execute(interaction, serversRconConfig, serverName) {
        ...
        try {
            const command = `broadcast ${broadcastMessage}`;
            ...
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({ content: 'Failed to send broadcast. Is the server online?', ephemeral: true });
            } else {
                await interaction.reply({ content: 'Failed to send broadcast. Is the server online?', ephemeral: true });
            }
        }
    }
};
