...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('unban')
        .setDescription('Unban a player from the Minecraft server')
        ...
        .addStringOption(option =>
            option.setName('player')
                .setDescription('Player name to unban')
                .setRequired(true)),
    async execute(interaction, serversRconConfig, serverName) {
        ...
        try {
            ...
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({ content: 'Failed to unban player. Is the server online?', ephemeral: true });
            } else {
                await interaction.reply({ content: 'Failed to unban player. Is the server online?', ephemeral: true });
            }
        }
    }
};
