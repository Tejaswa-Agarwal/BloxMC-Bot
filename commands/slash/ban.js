...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('player')
                .setDescription('Player name to ban')
                .setRequired(true)),
    async execute(interaction, serversRconConfig, serverName) {
        ...
        try {
            ...
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({ content: 'Failed to ban player. Is the server online?', ephemeral: true });
            } else {
                await interaction.reply({ content: 'Failed to ban player. Is the server online?', ephemeral: true });
            }
        }
    }
};
