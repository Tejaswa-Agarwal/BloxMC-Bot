...
module.exports = {
    ...
    async execute(interaction, serversRconConfig, serverName) {
        ...
        try {
            ...
            if (interaction.replied || interaction.deferred) {
                await interaction.editReply({ content: 'Failed to restart server. Is the server online?', ephemeral: true });
            } else {
                await interaction.reply({ content: 'Failed to restart server. Is the server online?', ephemeral: true });
            }
        }
    }
};
